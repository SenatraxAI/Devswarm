...
export function AgentPanel({ projectId, activeContext, setActiveContext, notifications }: AgentPanelProps) {
    ...
    const getStatusColor = (status: string) => {
        if (status.startsWith('Executing') || status === 'working') return 'bg-accent-primary shadow-[0_0_8px_rgba(168,85,247,0.5)]';
        switch (status) {
            case 'thinking': return 'bg-agent-thinking shadow-[0_0_8px_rgba(234,179,8,0.5)]';
            case 'speaking': return 'bg-agent-speaking shadow-[0_0_8px_rgba(6,182,212,0.5)]';
            case 'error': return 'bg-agent-error shadow-[0_0_8px_rgba(239,68,68,0.5)]';
            default: return 'bg-agent-idle';
        }
    };
    ...
    return (
        <aside className="w-full h-full flex flex-col bg-surface transition-colors duration-500">
            <div className="p-4 border-b border-surface-light flex items-center justify-between">
                ...
                <div className={`w-2 h-2 rounded-full ${isConnected ? 'bg-agent-success' : 'bg-agent-error animate-pulse'}`} />
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
                ...
                <button
                    ...
                    className={`w-full group relative bg-background border rounded-xl p-4 transition-all text-left mb-4 ${activeContext === 'general' ? 'border-accent-primary/50 bg-accent-primary/5' : 'border-surface-light hover:border-gray-700'}`}
                >
                    <div className="flex items-center space-x-3">
                        <div className="w-10 h-10 rounded-lg bg-surface border border-surface-light flex items-center justify-center">
                            <MessageSquare className={`w-5 h-5 ${activeContext === 'general' ? 'text-accent-primary' : 'text-gray-600'}`} />
                        </div>
                        <div>
                ...
                {agents.map((agent) => {
                    ...
                    return (
                        <div
                            ...
                            className={`cursor-pointer group relative bg-background border rounded-xl p-4 transition-all hover:bg-surface-light/40 ${isActive ? 'border-accent-primary/50 bg-accent-primary/5' : 'border-surface-light hover:border-gray-700'}`}
                        >
                            <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center space-x-3">
                                    <div className="relative">
                                        <div className="w-10 h-10 rounded-lg bg-surface border border-surface-light flex items-center justify-center">
                                            <User className={`w-5 h-5 ${isActive ? 'text-accent-primary' : 'text-gray-600'}`} />
                                        </div>
                                        <div className={`absolute -bottom-1 -right-1 w-3 h-3 rounded-full border-2 border-background ${getStatusColor(agent.status)} ${agent.status !== 'idle' ? 'animate-pulse' : ''}`} />
                                    </div>
                                    <div>
            ...
            <div className="p-4 border-t border-surface-light">
                <div className="bg-background rounded-lg p-3 border border-surface-light">
